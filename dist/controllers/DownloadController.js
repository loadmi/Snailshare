...
class DownloadController {
    ...
    showStartPage = async (req, res) => {
        try {
            const fileToken = req.query.file;
            if (!fileToken) {
    ...
    downloadFile = async (req, res) => {
        try {
            ...
            res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(file.metadata.originalName)}"`);
            // Start the download using the existing session
            await throttleService.startFileDownload(session, file.path, file.metadata.size);
    ...
    handleChallenge = async (req, res) => {
        try {
            ...
            const difficultyLevel = difficulty || DifficultyLevel.EASY;
